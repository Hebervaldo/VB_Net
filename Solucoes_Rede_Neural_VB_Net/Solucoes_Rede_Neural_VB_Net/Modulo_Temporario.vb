...
Module Modulo_Temporario
    Public Class clsBancoDados : Inherits Object
        ' Variável de classe
        Private Shared numInstanciasCriadas As Integer = 0
        ' Constantes
        Private Const cntErroMensagem As String = "Não há Erros."
        Private Const cntErroMensagemDR As String = "O Leitor de Dados (DataReader-dr) ainda não foi aberto."
        Private Const cntAlertaMensagemDR As String = "Não há mais registros disponíveis no Leitor de Dados (DataReader-dr)."
        ...
        Private da As New OleDbDataAdapter(strcmd, strcnn)
        Private dr As OleDbDataReader
        Private ds As New DataSet()
        Private intcntRegistroDR As Integer = 0
        Private intnumMaxRegistroDR As Integer = 0
        Private strNomeTabela As String = "Tabela"
        Private exErroMensagem As String = cntErroMensagem
        ...
        Public Sub New(ByVal strcnn As String, ByVal strcmd As String)
            setstrcnn = strcnn
            setstrcmd = strcmd ' Se a conexão estiver aberta use uma instrução SQL para selecionar os registros da tabela tblEmpregados
        ...
        Public ReadOnly Property getstrcnn() As String
            Get
                ' getstrcnn = strcnn
                Return strcnn
            End Get
        ...
        Public WriteOnly Property setstrcnn() As String
            Set(ByVal value As String)
                strcnn = value
            End Set
        End Property
        ' O Método a seguir tem por finalidade abrir uma conexão de dados.
        Public Overloads Function mtdAbrirConexaoBD() As Boolean
            Try
        ...
        Public Overloads Function mtdAbrirConexaoBD(ByVal strcnn As String) As Boolean
            setstrcnn = strcnn
            Return mtdAbrirConexaoBD()
        End Function
        ' Método que fecha a conexão aberta.
        Public Function mtdFecharConexaoBD() As Boolean
            Try
                cnn.Close()
        ...
        Public ReadOnly Property getstrcmd() As String
            Get
                ' getstrcmd = strcmd
                Return strcmd
            End Get
        ...
        Public WriteOnly Property setstrcmd() As String
            Set(ByVal value As String)
                strcmd = value
            End Set
        End Property
        ' O Método a seguir tem por finalidade executar o comando sql informado.
        Public Overloads Function mtdExecutarComando() As Boolean
            Try
                With cmd
                    .Connection = cnn
                    ' .Connection.ConnectionString = getstrcnn
                    .CommandType = CommandType.Text
                    .CommandText = getstrcmd
        ...
        Public Overloads Function mtdExecutarComando(ByVal strcmd As String) As Boolean
            setstrcmd = strcmd
            Return mtdExecutarComando()
        End Function
        ' Propriedade que resgata o valor do dataset contido na variável de instância ds.
        Public ReadOnly Property getAjustadorDados() As DataSet
            Get
        ...
        Public WriteOnly Property setAjustadorDados() As DataSet
            Set(ByVal value As DataSet)
                ds = value
            End Set
        End Property
        ' Método que resgata o cabeçalho da coluna especificada.
        Public Overloads Function mtdgetCabecalhoColunas(ByVal numcoluna As Integer) As String
        ...
        Public Overloads Sub mtdgetCabecalhoColunas(ByRef Colunas() As String)
            For contador As Integer = 0 To ds.Tables(0).Columns.Count - 1
        ...
        Public Overloads Function mtdgetCabecalhoColunas() As String()
            Dim comprimentoColunas As Integer = ds.Tables(0).Columns.Count
            Dim vetColunas(comprimentoColunas - 1) As String
            For contador As Integer = 0 To comprimentoColunas - 1
        ...
        Public Function mtdgetLeitorDadosEstatico(ByVal numColuna As Integer) As String
            Dim saida As String = String.Empty
            Try
                If (Not Object.ReferenceEquals(dr.GetValue(numColuna).ToString(), String.Empty)) Then 'If Not (dr.Item(numColuna).ToString Is String.Empty) Then
        ...
        Public Function mtdgetLeitorDadosDinamico(ByVal numColuna As Integer) As String
            Dim saida As String = String.Empty
            If mtdProximoRegistroDR() Then
                ' getLeitorDados = dr
                saida = dr.Item(numColuna).ToString
            Else
                saida = cntAlertaMensagemDR
            End If
            Return saida
        ...
        Public Sub mtdFecharLeitorDados()
            Try
        ...
        End Sub
        ' O Método a seguir fecha o Leitor de Dados.
        Public Function mtdVerificarLeitorDados() As Boolean
            Dim erro As Boolean = False
            Try
                If dr.IsClosed Then
                    erro = True
        ...
        Public ReadOnly Property getNomeTabela() As String
            Get
                Return strNomeTabela
            End Get
        End Property
        ' A Propriedade a seguir define o Nome da Tabela.
        Public WriteOnly Property setNomeTabela() As String
            Set(ByVal value As String)
                strNomeTabela = value
            End Set
        End Property
        Public ReadOnly Property getErroMensagem() As String
            Get
                Return exErroMensagem
            End Get
        End Property

        Private WriteOnly Property setErroMensagem() As String
            Set(ByVal value As String)
                exErroMensagem = value
            End Set
        End Property
        ...
        Public Function mtdAdaptadorDados() As Boolean
            ...
            Try
                With da
                    .SelectCommand.Connection.ConnectionString = getstrcnn
                    .SelectCommand.CommandText = getstrcmd
                    .Fill(getAjustadorDados, getNomeTabela)
                End With
        ...
        Public Function mtdProximoRegistroDR() As Boolean
            Dim erro As Boolean = False
            Try
                If dr.Read Then
                    intcntRegistroDR += 1
        ...
        Public Function mtdNumColunaDR() As Integer
        ...
        Public Function mtdNumMaxRegistroDR() As Integer
            Dim saida As Integer
            If Not dr.IsClosed Then
                mtdFecharLeitorDados()
            End If
            mtdDefinirLeitorDados()
            Do While mtdProximoRegistroDR()
        ...
        Public ReadOnly Property getNumeroRegistroDR() As Integer
            Get
                Return intcntRegistroDR
            End Get
        End Property
